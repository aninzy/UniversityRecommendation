...
def ProcessFinalData(user_data, college_rank):
    ...
    for i in range(size):
        ...
        try:
            ...
            result[name] = College_Info(
                rank, city, state, tuition, sat, accept_rate, debt, male_ratio)
        except Exception as ex:
            pass
    return result
...
def Recommendations(data):
    ...
    for i in range(len(names)):
        score[names[i]] = 0.15 * (1 - tuitions[i]) + 0.4 * \
            (1 - sats[i]) + 0.35*accept_rates[i] + 0.2*(1-ranks[i])
...
def main():
    ...
    user_data = extractRankingField('../WebScraped_data/csv/Undergrad/MERGED2016_17_PP.csv', [
